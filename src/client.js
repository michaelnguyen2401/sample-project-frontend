...
const resolvers = {
	Mutation: {
		...
		setDraftList: (_, { draft }, { cache }) => {
			...
			updatedDarft = {
				...draftList,
				items: draftList.items.find(i => i.id === draft.id)
					? draft.message === '[{"children":[{"text":""}]}]'
						? draftList.items.filter(i => i.id !== draft.id)
		...
		setStatusReadedMessage: (_, { readedMessage: { from, to } }, { cache }) => {
			...
			const findNotiMessageOfConversation = newNotiMessageList.items.find(
				noti => noti.from === from && noti.to === to
			)
